                    ...
                    <form>
                        <div class="row">
                            ...
                            <div class="col-lg-3 col-md-6 mt-1">
                                ...
                                <select v-model="item_type" class="form-select form-select-sm" id="item_type">
                                    <option value="">[ Pilih Jenis Transaksi ]</option>
                                    <option value="App\Models\Exam\Exam">Pembelian Latihan Soal</option>
                                    <option value="App\Models\Exam\ExamGroup">Pembelian Tryout</option>
                                    <option value="App\Models\Material\Module">Pembelian Modul/Materi</option>
                                    <option value="App\Models\Material\VideoModule">Pembelian Video Pembajaran</option>
                                    <option value="App\Models\Transaction\Voucher">Pembelian Membership</option>
                                    <option value="TopupBalance">Top Up Saldo</option>
                                </select>
                            </div>

                            <div class="col-lg-3 col-md-6 mt-1">
                                <label for="transaction_status">Status Transaksi</label>
                                <select v-model="transaction_status" class="form-select form-select-sm" id="transaction_status">
                            ...
                            <div class="col-lg-3 col-md-6 mt-1">
                                ...
                                <select v-model="payment_method" class="form-select form-select-sm" id="payment_method">
                                    <option value="">[ Pilih Metode Pembayaran ]</option>
                                    <option v-for="paymentMethod in $page.props.setting.payment_methods" :value="paymentMethod.code">{{ paymentMethod.name }}</option>
                                </select>
                            </div>
            ...
            <div class="card border-top border-0 border-3 border-primary">
                <div class="card-header">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="ms-auto">
                            <Link href="/admin/transactions/create" class="btn btn-primary btn-sm me-2">
                                <i class="bx bxs-plus-square"></i>Tambah Transaksi
                            </Link>
                        </div>
                    </div>
                </div>
                                    ...
                                    <td>
                                        <div class="dropdown">
											...
											<ul class="dropdown-menu" data-popper-placement="top-start" data-popper-reference-hidden="" data-popper-escaped="">
                                                <!-- <li><Link class="dropdown-item" :href="`/admin/transactions/${transaction.id}/invoice`">Invoice</Link></li> -->
                                                <li><Link class="dropdown-item" :href="`/admin/transactions/${transaction.id}`">Detail</Link></li>
                                                <li><a class="dropdown-item" href="#" @click.prevent="destroy(transaction.id)">Hapus</a></li>
											</ul>
										</div>
                                    </td>
                                </tr>
...
<script>
    ...
    export default {
        ...
        setup() {
            ...
            const item_type = ref((new URL(document.location)).searchParams.get('item_type') || "");
            const transaction_status = ref((new URL(document.location)).searchParams.get('transaction_status') || "");
            const payment_method = ref((new URL(document.location)).searchParams.get('payment_method') || "");
            ...
            const exportTransaction = () => {
                ...
                searchParams.append('item_type', item_type.value ?? '');
                searchParams.append('transaction_status', transaction_status.value ?? '');
                searchParams.append('payment_method', payment_method.value ?? '');
