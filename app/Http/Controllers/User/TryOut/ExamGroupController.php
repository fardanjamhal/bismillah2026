...
class ExamGroupController extends Controller
{
    ...
    { 
        ...
        try {
            ...
            if(Auth::user()->member_type == 2 && $examGroup && $setting->enable_tryout_sales == 1) {
                ...
                $checkMemberCategories = $this->checkMemberCategories(
                    ...
                    $examGroup->memberCategories->pluck('id')->toArray() // Mengonversi koleksi menjadi array
                );

                if($setting->tryout_sales_type == 1 && count($examGroup->userAccess) > 0) {
                    $checkAcess = true;
                }

                if($setting->tryout_sales_type == 2 && $checkMemberCategories == true) {
                    $checkAcess = true;
                }

                if($setting->tryout_sales_type == 3 && (count($examGroup->userAccess) > 0 || $checkMemberCategories == true)) {
                    $checkAcess = true;
                }
            ...
            if(!$examGroupUser) {
                ...
                $examGroupUser->end_time = Carbon::now()->addMinutes($examGroup->duration)->addSeconds(6);
    ...
    public function examShowQuestion($id, $section, Request $request)
    {
        try {
            ...
            if($examGroupUser->section < $section) {
                ...
                $examGroupUser->end_time = Carbon::now()->addMinutes($exam->duration)->addSeconds(6);
